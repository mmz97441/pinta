...
BEGIN
 ...
 IF jsonb_typeof(NEW.final_packages)='array' AND jsonb_array_length(NEW.final_packages)>0 THEN
  SELECT greatest(sum((value->>'poids')::numeric),sum((value->>'dimL')::numeric*(value->>'dimW')::numeric*(value->>'dimH')::numeric/divisor)) INTO NEW.poids_facturable FROM jsonb_array_elements(NEW.final_packages);
 ELSIF NEW.fin_l IS NOT NULL AND NEW.fin_w IS NOT NULL AND NEW.fin_h IS NOT NULL AND NEW.fin_p IS NOT NULL THEN NEW.poids_facturable:=greatest(NEW.fin_p,NEW.fin_l*NEW.fin_w*NEW.fin_h/divisor); END IF;
 RETURN NEW;
END; $$;
