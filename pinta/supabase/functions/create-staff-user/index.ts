...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (typeof email !== 'string' || !email.includes('@') || typeof password !== 'string' || password.length < 12 || !nom?.trim() || !['directeur','vice_directeur','logisticien','preparateur'].includes(role)) throw new HttpError(400, 'Nom, email, rôle valide et mot de passe de 12 caractères minimum requis');
    const created = await db.auth.admin.createUser({ email: email.trim(), password, email_confirm: true, user_metadata: { nom: nom.trim(), prenom: prenom || '' } });
    ...
    return json({ success: true, userId: created.data.user.id });
