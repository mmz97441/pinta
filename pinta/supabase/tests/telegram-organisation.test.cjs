...
async function scenario({attachment=false,unsupported=false,reply=false,ambiguous=false,ackFails=false,invoiceFails=false}={}){
 ...
 q.upsert=q.update=value=>{mutation=value;return q;};const resolve=()=>{
  ...
  if(table==='messages'&&!mutation)return{data:{id:'fixture-message'},error:null};
 ...
 const run=await webhook(db,fetch);const message={message_id:123,date:1789000000,chat:{id:42,type:'private'},from:{id:42},...(attachment?{caption:'Document complémentaire',document:{file_id:'test-file',file_size:13,file_name:unsupported?'archive.zip':'document.pdf'}}:{text:'Ma réponse'}),...(reply?{reply_to_message:{message_id:90}}:{})};
