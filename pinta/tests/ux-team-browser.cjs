...
async function assertFocusedMeasurementVisible(dialog, carton) {
  ...
  const header = await dialog.getByRole('heading', { name: 'Réceptionner un colis', exact: true }).locator('..').locator('..').boundingBox();
...
async function run() {
  ...
  try {
    for (const mobile of [false, true]) {
      ...
      assert.equal(await f.page.getByLabel('Responsable', { exact: true }).inputValue(), 'mine');
      await f.page.getByLabel('Responsable', { exact: true }).selectOption('');
      ...
      const chat = f.page.getByRole('dialog', { name: 'Conversation du dossier', exact: true });
      await chat.waitFor();
      await f.page.keyboard.press('Shift+Tab');
      assert.equal(await chat.evaluate(node => node.contains(document.activeElement)), true);
      ...
      await f.page.getByRole('button', { name: mobile ? 'Réceptionner un colis' : 'Nouveau colis', exact: true }).click();
      await dialog.getByLabel('Client', { exact: true }).fill('Nouveau test');
      ...
      await dialog.getByLabel('Poids à réception (kg) · carton 1', { exact: true }).fill('1.25');
      ...
      await measure(dialog, 1, [12, 23, 34, 1.25]);
      if (mobile) await assertFocusedMeasurementVisible(dialog, 1);
      ...
      results.push({ viewport: mobile ? '390x844' : '1440x1000', filters: 'PASS', nextDossier: 'PASS', scannerFocus: 'PASS', receiptCartons: 'PASS', stickyAction: 'PASS', focusedMeasurementUnobscured: mobile ? 'PASS new receipt and attachment' : 'mobile-only check', nestedChatKeyboard: 'PASS', mandatoryReceiptMeasurements: 'PASS', newClientReceipt: 'PASS', appendMeasurementsPreserved: 'PASS', cartonWithoutTracking: 'PASS', receptionAndPackingSeparated: 'PASS', prohibitedItemsPersistence: 'PASS', axeReceiptAndDarkFile: 'PASS', darkOverflow: 'PASS', pageErrors: f.errors, unexpectedNetwork: f.networkDenied });
