...
(async () => {
  ...
  try {
    ...
    const firstArticle = page.getByRole('article', { name: 'Facture Boutique A', exact: true });
    await firstArticle.locator('canvas[data-rendered="true"]').waitFor();
    assert.equal(await firstArticle.evaluate(element => element === document.activeElement), true);
    ...
    await firstArticle.getByRole('button', { name: 'Valider la facture', exact: true }).click();
    await firstArticle.getByText('Validée', { exact: true }).waitFor();
    ...
    assert.equal(await page.getByRole('tab', { name: 'Document', exact: true }).getAttribute('aria-selected'), 'true');
