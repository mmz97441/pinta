...
(async () => {
  ...
  try {
    ...
    await page.getByRole('tab', { name: 'Articles et vérification', exact: true }).click();
    await firstArticle.getByRole('button', { name: 'Valider cette facture et ses articles', exact: true }).click();
    await page.getByTestId('invoice-feedback').filter({ hasText: 'facture et articles validés et enregistrés' }).waitFor();
