...
export default function TeamWorkView() {
  ...
  const rows = sortWorkActions(all.filter(action => (!owner || (owner === 'unassigned' ? !action.assignee_id : action.assignee_id === (owner === 'me' ? auth?.u?.id : owner)))
    && (!search || [dossiers.get(action.colis_id)?.ref, clientMap.get(dossiers.get(action.colis_id)?.clientId)?.nom, action.action_hint].join(' ').toLocaleLowerCase('fr').includes(search))
  ...
  return <main className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
    ...
    <details><summary className="min-h-11 cursor-pointer py-3 text-sm font-semibold">Disponibilité et charge de l’équipe ({teamUsers.filter(user => user.actif !== false).length})</summary><section aria-label="Charge et disponibilité" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{teamUsers.filter(user => user.actif !== false || all.some(action => action.assignee_id === user.authId)).map(user => {
    ...
    <nav aria-label="Priorités de l’équipe" className="flex flex-wrap gap-2">{[['unassigned', 'Sans responsable', all.filter(a => !a.assignee_id).length], ['blocked', 'À débloquer', all.filter(a => a.blocked_reason).length], ['handoff', 'Relais à accepter', all.filter(a => a.handoff_to).length]].map(([key, label, count]) => <button key={key} className="min-h-11 rounded-xl border px-3 text-sm font-semibold" onClick={() => setParams(key === 'unassigned' ? { owner: key } : { exception: key })}>{label} ({count})</button>)}</nav>
    <section className="rounded-xl border border-slate-200 bg-white p-4"><label className="mb-3 block text-sm font-semibold">Rechercher une EXP ou un client<input value={params.get('q') || ''} onChange={event => update('q', event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border px-3 font-normal" /></label><details><summary className="min-h-11 cursor-pointer py-3 text-sm font-semibold">Filtrer les tâches{[owner, mission, state, exception].some(Boolean) ? ' · filtres actifs' : ''}</summary><div className="flex flex-wrap gap-3">
    ...
    </div></details>{[owner, mission, state, exception, search].some(Boolean) && <button onClick={() => setParams({})} className="min-h-11 text-sm underline">Afficher toutes les tâches</button>}<p className="mt-4 text-sm text-slate-600">{totals.actions} actions · {totals.dossiers} dossiers · {totals.cartons} cartons reçus</p>
