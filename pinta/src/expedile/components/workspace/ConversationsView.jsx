  ...
  return <main className="flex h-[calc(100dvh-8rem)] min-h-[520px] flex-col p-3 md:p-5">
    ...
    <div className="grid min-h-0 flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[280px_minmax(0,1fr)_300px]">
      ...
      <section className={`${selected || selectedInbox ? 'flex' : 'hidden lg:flex'} min-h-0 flex-col overflow-hidden`}>
        ...
        {selectedInbox ? <div className="space-y-4 overflow-y-auto p-5"><p className="text-sm text-slate-600">Le client n’a pas encore précisé le dossier concerné. Rattachez le message après vérification de son contenu.</p><p className="whitespace-pre-wrap rounded-xl bg-slate-50 p-4 text-sm">{selectedInbox.texte || 'Document reçu sur Telegram'}</p><InboxAttachment item={selectedInbox} /><label className="block text-sm font-semibold">Dossier du client<select value={assignment} onChange={event => setAssignment(event.target.value)} className="mt-2 min-h-11 w-full rounded-lg border px-2"><option value="">Choisir un dossier</option>{data.filter(item => item.clientId === (selectedInbox.client_id || selectedInbox.clientId)).map(item => <option key={item.id} value={item.id}>{item.ref} · {item.desc || item.statut} · {receptionCartonManifest(item).nbColis} carton(s) · {item.dateReception ? new Date(item.dateReception).toLocaleDateString('fr-FR') : 'date à préciser'}</option>)}</select></label><button disabled={!assignment || busy || !can('perm_comm_telegram')} onClick={() => ask('Rattacher ce message ?', `Le message et son document seront rattachés à ${data.find(item => item.id === assignment)?.ref || 'cette expédition'} pour ${name(selectedClient)}.`, assign, { okLabel: 'Confirmer le rattachement' })} className="min-h-11 rounded-lg bg-slate-900 px-4 text-sm font-semibold text-white disabled:opacity-40">{busy ? 'Rattachement…' : 'Rattacher au dossier'}</button>{error && <p role="alert" className="text-sm text-red-700">{error}</p>}</div> : <>
          <div className={`${contextVisible ? 'hidden xl:flex' : 'flex'} min-h-0 flex-1 flex-col`}>{action && <details className="border-b px-3 py-1"><summary className="min-h-11 cursor-pointer py-2 text-sm font-semibold">Réalise cette tâche : {staffName(action.assignee_id, teamUsers)}</summary><WorkActionControls action={action} returnTo={returnTo} compact /></details>}<ChatPanel colis={selected} client={selectedClient} embedded active={!contextVisible} /></div>
