...
export default function ClientAccueil() {
  ...
  return <div className="anim-fade space-y-6">
    <header className="border-b border-slate-200 pb-4"><p className="text-sm text-slate-500">Mon espace Expedîle{destination ? ` · ${destination.nom}` : ''}</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Bonjour {getPrenom(authCl) || 'Client'}</h1><p className="mt-2 text-sm text-slate-600">{grouped.todo.length ? `${grouped.todo.length} expédition${grouped.todo.length > 1 ? 's attendent' : ' attend'} une action de votre part.` : 'Aucune action urgente demandée. Vous pouvez suivre vos expéditions ci-dessous.'}</p></header>
    ...
    {SECTIONS.map(([key, title, description, Icon]) => <section key={key} aria-labelledby={`client-home-${key}`} className="space-y-3"><div className="flex items-center gap-2"><Icon size={18} className="text-slate-600" /><h2 id={`client-home-${key}`} className="text-base font-bold text-slate-900">{title}</h2><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700">{grouped[key].length} expédition{grouped[key].length > 1 ? 's' : ''}</span></div><p className="text-sm text-slate-600">{description}</p>{grouped[key].length ? <div className="grid gap-3 md:grid-cols-2">{grouped[key].slice(0, key === 'history' ? 2 : 6).map(colis => <ClientShipmentCard key={colis.id} colis={colis} client={authCl} onOpen={() => navigate(`/colis/${colis.id}`)} />)}</div> : <p className="rounded-xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">{key === 'todo' ? 'Vous êtes à jour.' : 'Aucune expédition dans cette rubrique.'}</p>}{(grouped[key].length > (key === 'history' ? 2 : 6) || key === 'history') && <button className="min-h-11 rounded-xl border border-slate-200 px-4 text-sm font-semibold brand-t" onClick={() => { setColisFilter(key); navigate('/colis'); }}>Voir {key === 'history' ? 'tout mon historique' : 'toutes ces expéditions'}</button>}</section>)}
  </div>;
}
