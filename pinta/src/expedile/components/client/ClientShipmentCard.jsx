...
export default function ClientShipmentCard({ colis, client, onOpen }) {
  ...
  return <button onClick={onOpen} className="card min-h-11 w-full rounded-2xl p-4 text-left transition-colors hover:bg-slate-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-600">
    ...
    <p className="mt-3 flex items-center gap-2 text-sm text-slate-600"><Package size={15} />{manifest.count} carton{manifest.count > 1 ? 's' : ''} réceptionné{manifest.count > 1 ? 's' : ''}</p>
    <p className="mt-2 text-sm font-semibold brand-t">{state.section === 'todo' ? state.action : state.journey.label}</p>
    <p className="mt-1 text-sm text-slate-600">{state.section === 'todo' ? state.journey.label : state.journey.next}</p>
    {state.journey.waiting && <p className="mt-2 text-sm text-slate-600">{colis.attenteClientMotif}{colis.attenteClientUntil ? ` · Réexamen ${state.journey.reviewDue ? 'prévu depuis' : 'prévu le'} ${new Date(colis.attenteClientUntil).toLocaleDateString('fr-FR')}` : ''}</p>}
