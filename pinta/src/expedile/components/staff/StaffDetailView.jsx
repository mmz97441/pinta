        ...
        return <div className={`min-w-0 space-y-5 ${workspace ? "mx-auto w-full max-w-6xl" : ""}`}>
          ...
          {!isPro && <div id="quote-articles" className="scroll-mt-24"><Section title="Récapitulatif des articles du dossier" icon={Package} color={borderColor}>
            ...
            <div className="space-y-3">{(sel.lignes || []).filter(line => !line.factureId).map((line) => <div key={line.id} className="space-y-2 border-b border-gray-100 pb-3">
              ...
              <p className="text-xs text-slate-600">Les articles liés aux factures se vérifient dans leur facture ci-dessus. Les lignes ci-dessous ont été saisies séparément : contrôlez leur présence dans les factures pour éviter un double comptage.</p>
          ...
          <div id="quote-review" data-testid="quote-action-bar" className="-mx-1 border-t border-slate-200 bg-white px-3 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-[0_-3px_12px_rgba(0,0,0,0.06)]">
